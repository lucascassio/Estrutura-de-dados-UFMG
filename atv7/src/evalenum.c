...
// Função principal do quicksort com inserção para partições pequenas
// Função principal do quicksort com inserção para partições pequenas
void quickSortIns(int *A, int l, int r, sortperf_t *s) {
    if (r - l < 10) {
        ...
        int i, j;
        partition(A, l, r, &i, &j, s);
        inccalls(s, 1); // Incrementa a chamada de função aqui
...
// Função principal do quicksort com mediana de três e inserção para partições pequenas
